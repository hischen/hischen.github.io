...
最近几个月参加了由KDD2020、阿里巴巴集团等举办的天池大数据竞赛算法大赛KDD Cup 2020 Challenges for Modern E-Commerce Platform: Multimodalities Recall 赛题，在参赛的1433支队伍里获得16名的成绩，这里简单复盘一下我们的方案。
...
模型融合
	训练了多个不同类型的bert，将每个模型最后的输出结果直接相加作为最终结果。这里用到的模型有bert-6，bert-9,  Roberta-6, Roberta-9. 这里bert-6代表6层的bert模型。
